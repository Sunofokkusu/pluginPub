...
add_action( 'admin_menu', 'adtest_add_metabox' );

function adtest_add_metabox() {
	
	//Box pour ajouter l'image au format pour mobile
	add_meta_box(
		'adtest_metabox_imgmobile', // id metabox 
		'Images de couverture', // titre
		'adtest_image_mobile_callback', // fonction de callback 
	...
	add_meta_box(
		'adtest_metabox', // id metabox 
		'Informations sur la publicité', // titre
		'adtest_metabox_callback', // fonction de callback 
		'regie_publicitaire', // post type 
		'normal', // position 
		'default' // priorité
	);
	...
	add_meta_box(
		'adtest_metabox_statut', // id metabox 
		'Statut', // titre
		'adtest_statut_callback', // fonction de callback 
		'regie_publicitaire', // post type 
		'side', // position 
		'default' // priorité
	);
	...
	add_meta_box(
		'adtest_metabox_ref', // id metabox 
		'Référence', // titre
		'adtest_ref_callback', // fonction de callback 
		'regie_publicitaire', // post type 
		'side', // position 
		'default' // priorité
	);
...
function adtest_image_mobile_callback($post){ //images
	include plugin_dir_path( __FILE__ ) . './images.php';
}

function adtest_metabox_callback( $post ) { //paramètres
	include plugin_dir_path( __FILE__ ) . './form.php';
}

function adtest_statut_callback($post){ //statut
	include plugin_dir_path( __FILE__ ) . './statut.php';
}

function adtest_ref_callback($post){ //référence
	include plugin_dir_path( __FILE__ ) . './ref.php';
}

/*Fonction permettant de sauvegarder le contenu des metabox*/
add_action( 'save_post', 'save_meta_boxes',1 );
...
function save_meta_boxes( $post_id ) {
	...
	$options = get_option('ad_options', array()); 
	foreach($arr_post as $post){
		update_post_meta( get_the_ID(), 'ref', sanitize_text_field($options['ad_prefixe']."-".get_the_date('mY')."-".get_the_ID().$options['ad_suffixe']) );
	}
	
	//Sauvegarde l'image ajoutée par la version mobile
	if ( ! current_user_can( 'edit_posts', $post_id ) ){ return 'not permitted'; }
    if (isset( $_POST['custom_postimage_meta_box_nonce'] ) && wp_verify_nonce($_POST['custom_postimage_meta_box_nonce'],'adtest_metabox_imgmobile' )){
...
add_action('post_updated_messages','show_error',1000);
...
function show_error(){
	...
	if($debut > $fin){ //si la date de début est postérieure
		...
		<div class="notice error my-acf-notice" >
			<p><strong><?php _e( 'La date de début ne peut pas être postérieure à la date de fin.', 'adtest' ); ?></strong></p>
		</div>
		<?php	
	}
}

add_action('init','session');
function session(){
	if ( !session_id() ) {
		session_start();
	}
...
add_action( 'save_post', 'dates_disponibles', 1001);
...
function dates_disponibles($post_id){
	//l'action ne s'exécute pas si l'on met le poste à la corbeille ou si on le restaure
    if(
        isset($_REQUEST['action']) &&
        ( $_REQUEST['action'] == 'trash' || $_REQUEST['action'] == 'untrash')
...
add_action('post_updated_messages','show_error_dates_dispo',1002);
